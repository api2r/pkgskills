...
with `stbl::expect_pkg_error_snapshot()` from `stbl`, which captures both the
error class hierarchy and the user-facing message in one snapshot:
...
```r
withr::local_options(list(pkgskills.verbose = TRUE))
