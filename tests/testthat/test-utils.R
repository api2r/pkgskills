...
test_that(".use_template_as_is() errors on non-logical open (#45)", {
  local_pkg()
  stbl::expect_pkg_error_classes(
...
test_that(".use_template_as_is() writes template content verbatim (#45)", {
  proj_dir <- local_pkg()
...
test_that(".use_template_as_is() calls edit_file when open = TRUE (#45)", {
  proj_dir <- local_pkg()
  edited <- character(0)
  ...
  expect_identical(edited, fs::path(proj_dir, "AGENTS.md"))
})
